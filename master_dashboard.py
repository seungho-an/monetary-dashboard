...
import matplotlib
matplotlib.use('Agg') # Use 'Agg' backend for non-GUI server
...
try:
    ...
    # --- 3d. Yahoo Finance Data ---
    print("Fetching Yahoo Finance data...")
    # Fetch all tickers, group by ticker to handle complex structure
    asset_data_raw = yf.download(yahoo_tickers, start=start_date_long, end=end_date, group_by='ticker')
...
df_market_risk = df_market_risk.ffill()
...
df_global_risk = df_global_risk.ffill()
...
try:
    ...
    latest_values['elasticity'] = get_last_value(elasticity_df.get(f'{rolling_window_weeks_list[-1]}-Wk Elasticity'))
...
# plt.show() # <-- Commented out for automated run. Uncomment for local testing.
...
try:
    ...
    if red_count >= 4: 
        ...
        if status_results.get('t10y2y') and status_results['t10y2y'][0] == STATUS_COLORS['red']:
             report_lines.append("  > The Yield Curve is inverted, signaling a future recession risk.")
        if status_results.get('houst_yoy') and status_results['houst_yoy'][0] == STATUS_COLORS['red']:
             report_lines.append("  > Housing Starts are down significantly, strongly signaling a future recession.")
        if status_results.get('nfci') and status_results['nfci'][0] == STATUS_COLORS['red']:
...
def generate_html_page(report_file, image_files, status):
    ...
    try:
        with open(report_file, 'r', encoding='utf-8') as f:
            # Convert newlines to <br> and spaces to &nbsp; for HTML
...
<head>
    ...
    <style>
        ...
        /* 1-column layout for the last dashboard */
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="summary">
                <details open>
                    <summary>Click to expand/collapse the latest automated analysis</summary>
